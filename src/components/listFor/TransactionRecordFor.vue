    ...
    <div class="success_info_body" v-show="showFlag">
      ...
      <div class="table_body">
        <div class="table_th">
          ...
          <div class="tr2 th_style">交易数量</div>
          <div class="tr3 th_style">交易金额</div>
        ...
        <div class="table_main_body">
          <div class="table_inner_body" :style="{top: tableTop + 'px'}">
            <div class="table_tr" v-for="(item,index) in tableList" :key="index">
              ...
              <div class="tr2 tr">{{item.quantity}}</div>
              <div class="tr3 tr" v-if="item.stock!='-'">{{item.transactionAmount}}</div>
...
export default {
  ...
  methods: {
    ...
    bsGetProductProcess() {
      listTransactionRecord().then(response => {
        this.tableList = response.rows.filter(item => {
          return new Date(item.transactionTime).toDateString() === new Date().toDateString();
        });
        this.showFlag = true;
        this.tableActionFun();
      });
